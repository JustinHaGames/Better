...
public class PlayerMovement : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        //Tell Gamemanger when player Vel.y is negative
        if (vel.y <= 0)
        {
            GameManager.instance.playerVelDown = true;
        } else
        {
            GameManager.instance.playerVelDown = false;
        }
        ...
        if (GameManager.instance.hookshotUnlocked)
        {
            ...
            if ((Input.GetButtonDown("Fire2")) && !shot && mashNumber <= 0)
            {
                if (lastR && !spawnedHookShot)
                {
                    ...
                    spawnedHookShot = true;
                    shot = true;
                }

                if (lastL && !spawnedHookShot)
                {
                    Instantiate(hookShot, new Vector3(transform.position.x - .25f, transform.position.y + .25f, transform.position.z), Quaternion.Euler(new Vector3(0, 0, 45)));
                    spawnedHookShot = true;
                    shot = true;
    ...
    void OnCollisionEnter2D(Collision2D coll)
    {
        ...
        if (coll.gameObject.tag == "Enemy")
        {
            ...
            if (transform.position.y >= collidedObject.transform.position.y + 1f || pull)
            {
                ...
                slideVel = 0;
                vel.y = boxBoost;
                audio.PlayOneShot(boxJumpSFX, .75f);
                if (shotHook != null)
                {
                    shotHook.SendMessage("DestroySelf", SendMessageOptions.DontRequireReceiver);
                }
    ...
    void OnTriggerStay2D(Collider2D coll)
    {
        if (coll.gameObject.tag == "Water")
        {
            waterSpeed = 3;
            swim = true;
            DreamManager.instance.switchSpeed = true;
        }
    }
    ...
    void OnTriggerExit2D(Collider2D coll)
    {
        if (coll.gameObject.tag == "Water")
        {
            waterSpeed = 1;
            swim = false;
            DreamManager.instance.switchSpeed = false;
        }
    }
